...
int main(int argc, char* argv[])
{
    ...
    sphere sph1(point3(8, 7, -20), 4, color(0, 0, 0), color(1., 1., 1.), MaterialType::specular);
    sphere sph2(point3(-10.5, 5, -20), 5, color(0, 0, 0), color(1., 1., 1.), MaterialType::Refract);
	...
	Shapes shapes(std::make_shared<sphere>(sph1));
...
#pragma omp parallel for schedule(dynamic, 1) // OpenMP
    for (int y = 0; y < image_height; y++)
    {
        ...
        std::cerr << "\rRendering (" << SPP * 4 << " spp) " << 100. * y / (image_height - 1) << "%";
